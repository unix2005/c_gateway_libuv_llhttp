...
#define _GNU_SOURCE      // 启用 GNU 扩展（如 syscall, usleep 等）
#define _DEFAULT_SOURCE  // 兼容旧版本 BSD
#define _POSIX_C_SOURCE 200809L  // POSIX.1-2008

// ==================== 标准 C 库 ====================
#include <stdio.h>       // 标准输入输出
#include <stdlib.h>      // 标准库函数（malloc, free, exit 等）
#include <string.h>      // 字符串处理
#include <strings.h>     // BSD 字符串函数（strcasecmp 等）
#include <ctype.h>       // 字符类型判断
#include <math.h>        // 数学函数
#include <time.h>        // 时间函数
#include <errno.h>       // 错误号
#include <limits.h>      // 限制常量
...
// ==================== POSIX 和系统接口 ====================
#include <unistd.h>      // POSIX API（read, write, sleep, gettid 等）
...
#include <sys/un.h>      // Unix 域 socket
#include <sys/ioctl.h>   // I/O 控制
#include <sys/mman.h>    // 内存映射
#include <sys/select.h>  // select 系统调用
#include <sys/epoll.h>   // epoll 事件通知
#include <sys/prctl.h>   // 进程控制
...
// ==================== 网络相关 ====================
#include <netinet/in.h>  // Internet 地址结构
#include <netinet/tcp.h> // TCP 协议选项
#include <netinet/ip.h>  // IP 协议
#include <arpa/inet.h>   // Internet 操作函数（inet_pton 等）
...
#include <langinfo.h>    // 语言信息
#include <wordexp.h>     // 单词展开
...
#ifdef HAVE_OPENSSL
    SSL *ssl;                            ///< SSL 连接句柄（仅当启用 HTTPS 时）
    BIO *ssl_bio;                        ///< SSL BIO（连接 libuv 和 OpenSSL）
    
    // SSL 读取缓冲区（存储从 TCP 读取的加密数据）
    char *ssl_read_buffer;               ///< SSL 读缓冲区
    size_t ssl_read_len;                 ///< SSL 读缓冲区已使用长度
    size_t ssl_read_capacity;            ///< SSL 读缓冲区容量
    
    // SSL 写入缓冲区（存储要发送到 TCP 的加密数据）
    char *ssl_write_buffer;              ///< SSL 写缓冲区
    size_t ssl_write_len;                ///< SSL 写缓冲区已使用长度
    
    int ssl_handshake_state;             ///< SSL 握手状态 (0=未开始，1=进行中，2=完成)
#endif
} client_ctx_t;
...
// === SSL/TLS 全局变量（仅当启用 HTTPS 时） ===
#ifdef HAVE_OPENSSL
#include <openssl/ssl.h>
#include <openssl/err.h>
#include <openssl/bio.h>

extern SSL_CTX* g_ssl_ctx;   ///< 全局 SSL 上下文
extern BIO_METHOD* g_bio_method;  ///< 全局 BIO 方法
...
/**
 * @brief 初始化 SSL BIO 方法
 * 
 * 创建自定义的 BIO 方法，用于连接 libuv 和 OpenSSL。
 * 
 * @return int 0 表示成功，-1 表示失败
 */
int init_ssl_bio(void);

/**
 * @brief 清理 SSL BIO 方法
 */
void cleanup_ssl_bio(void);
